...
private:
	...
	std::vector<point_t>				convertNodeToPoint(std::list<int> nodePath);
	static bool							isVisited(int index, std::vector<int> const & visited);
	static std::pair<int, double>		getMinNode(std::vector<std::pair<int, double>> const & path, std::vector<int> const & visited);
