...
void Pathfinding::addObstacleAdjList(rectangle_t const & obstacle, size_t index) {
	if (!isHitObstacle(obstacle.point[0], obstacle.point[1], obstacle)) {
		_obstaclesGraph.addEdge(index * 4, index * 4 + 1,
			calculateDist(obstacle.point[0], obstacle.point[1]));
	} if (!isHitObstacle(obstacle.point[1], obstacle.point[2], obstacle)) {
		_obstaclesGraph.addEdge(index * 4 + 1, index * 4 + 2,
			calculateDist(obstacle.point[1], obstacle.point[2]));
	} if (!isHitObstacle(obstacle.point[2], obstacle.point[3], obstacle)) {
		_obstaclesGraph.addEdge(index * 4 + 2, index * 4 + 3,
			calculateDist(obstacle.point[2], obstacle.point[3]));
	} if (!isHitObstacle(obstacle.point[3], obstacle.point[0], obstacle)) {
		_obstaclesGraph.addEdge(index * 4 + 3, index * 4,
			calculateDist(obstacle.point[3], obstacle.point[0]));
	}
}
		...
		for (size_t j = 0; j < 4; ++j) {
			if (_obstacles.size() > 2) {
				if (!isHitObstacle(lhs.point[i], rhs.point[j], lhs, rhs)
					...
					_obstaclesGraph.addEdge(lhs.id * 4 + i, rhs.id * 4 + j,
						calculateDist(lhs.point[i], rhs.point[j]));
				}
			} else {
	...
	for (size_t i = 0; i != _obstacles.size(); ++i) {
		for (size_t j = 0; j < 4; ++j) {
			if (!isHitItself(cp.position, _obstacles[i].point[j], _obstacles[i], j)
				...
				graph.addEdge(cp.graphIndex, i * 4 + j,
					calculateDist(cp.position, _obstacles[i].point[j]));
			}
		}
	}
